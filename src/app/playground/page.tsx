...
const LANG_OPTIONS = [
  ...
  { value: 'cpp',        label: 'C++ (GCC 11 / C++17)', ext: 'cpp'  },
];
...
export default function PlaygroundPage() {
  ...
  return (
    <div className="flex flex-col bg-slate-50" style={{ height: 'calc(100vh - 56px)' }}>
      <div className="flex items-center gap-3 px-4 py-2 bg-white border-b border-slate-200 shrink-0">
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="hidden sm:block">
            ...
            <p className="text-[10px] text-slate-400 leading-tight">Real server-side execution via containerized runtimes.</p>
          </div>
        </div>
        ...
        <select
          ...
          className="input text-xs h-9 py-0 w-48"
        >
          {LANG_OPTIONS.map(l => (
        ...
        <div className="flex items-center gap-2 ml-auto">
          <button
            onClick={handleReset}
            className="btn-ghost text-xs h-9 px-2.5 flex items-center gap-1.5 text-slate-500"
          ...
          <button
            onClick={handleRun}
            disabled={running}
            className="btn-primary text-xs h-9 px-5 flex items-center gap-2 disabled:opacity-60"
          >
            {running
      ...
      <div className="flex flex-col flex-1 gap-2 p-3 overflow-hidden min-h-0">
        <div className="flex-1 rounded-xl overflow-hidden border border-slate-200 bg-white shadow-xs min-h-0 flex flex-col">
          <div className="px-4 py-2 border-b border-slate-200 bg-slate-50 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2 text-xs text-slate-500">
              ...
              <span className="text-slate-400">Tab size: 4 spaces</span>
            </div>
        ...
        <div className={`bg-white border border-slate-200 rounded-xl shadow-xs flex flex-col shrink-0 transition-all duration-200 ${
          ...
          <div className="flex items-center border-b border-slate-200 px-2 shrink-0 bg-slate-50 rounded-t-xl">
            {([
              { key: 'stdin'  as const, label: 'Standard Input (stdin)', icon: Terminal },
              { key: 'output' as const, label: 'Output Console',         icon: Zap      },
