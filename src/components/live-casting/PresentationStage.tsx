...
export default function PresentationStage({
  ...
  if (isScreenSharing) {
    return (
      <div className="relative w-full h-full min-h-[320px] sm:min-h-[460px] bg-slate-950 rounded-2xl border border-slate-800/80 overflow-hidden flex flex-col shadow-2xl">
        {/* Top Presentation Info Banner */}
        <div className="px-3 sm:px-4 py-2 sm:py-2.5 bg-slate-900/90 border-b border-slate-800 flex items-center justify-between z-20">
          <div className="flex items-center gap-2 sm:gap-2.5 min-w-0 pr-2">
            ...
            <div className="min-w-0">
              <span className="text-[11px] sm:text-xs font-bold text-slate-100 flex items-center gap-1 sm:gap-1.5 truncate">
                {isPresenter ? 'You are sharing' : `${roomState?.presenterName || 'Presenter'} is sharing`}{' '}
                <span className="text-violet-400 font-extrabold truncate">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            {isPresenter ? (
              <button
                ...
                className="px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl font-bold text-[10px] sm:text-xs bg-rose-600/20 text-rose-300 hover:bg-rose-600/30 border border-rose-500/40 flex items-center gap-1 sm:gap-1.5 transition-colors"
              >
                <Square className="w-2.5 h-2.5 sm:w-3 sm:h-3 fill-current" />
                <span>Stop Sharing</span>
              </button>
            ) : (
              <span className="text-[9px] sm:text-[10px] font-semibold px-2 py-0.5 rounded-full bg-emerald-950 text-emerald-300 border border-emerald-800">
                LIVE STREAM
              </span>
            )}
        ...
        <div className="flex-1 relative bg-black flex items-center justify-center overflow-hidden">
          {activeScreenStream ? (
            ...
            <div className="flex flex-col items-center justify-center p-4 sm:p-8 text-center space-y-2">
              <Loader2 className="w-6 h-6 sm:w-8 sm:h-8 text-violet-400 animate-spin" />
          ...
          {/* Floating Camera Picture-in-Picture (PIP) Tile */}
          {presenterCamStream && (
            <div className="absolute bottom-2 right-2 sm:bottom-4 sm:right-4 z-30 w-28 h-20 sm:w-44 sm:h-32 rounded-xl sm:rounded-2xl overflow-hidden bg-slate-900 border-2 border-slate-700/80 shadow-2xl flex flex-col group">
              <video
                ref={pipVideoRef}
                autoPlay
                playsInline
              ...
              <div className="absolute bottom-1 left-1 right-1 px-1.5 py-0.5 rounded bg-black/70 backdrop-blur-sm flex items-center justify-between text-[8px] sm:text-[10px] text-slate-200">
                <span className="font-semibold truncate">
                  {roomState?.presenterName || 'Presenter'}
                </span>
                <div className="flex items-center gap-1">
                  {micOn ? (
                    <Mic className="w-2 h-2 sm:w-2.5 sm:h-2.5 text-emerald-400" />
                  ) : (
                    <MicOff className="w-2 h-2 sm:w-2.5 sm:h-2.5 text-rose-400" />
                  )}
                </div>
              </div>
            </div>
          ...
          {isPresenter && (
            <div className="absolute top-2 sm:top-4 left-1/2 -translate-x-1/2 z-30 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full bg-slate-950/90 border border-violet-500/50 text-[10px] sm:text-[11px] text-slate-200 shadow-xl backdrop-blur-md flex items-center gap-1.5 sm:gap-2 max-w-[90%] text-center">
              <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-emerald-400 animate-pulse shrink-0" />
              <span className="truncate">You are presenting your screen to the room.</span>
            </div>
          )}
        </div>
      </div>
  ...
  if (presentationType === 'code-editor' || presentationType === 'sparkcode-workspace') {
    return (
      <div className="relative w-full h-full min-h-[320px] sm:min-h-[460px] flex flex-col gap-2">
        {/* Top Presentation Info Banner */}
        <div className="px-3 sm:px-4 py-2 bg-slate-900/90 rounded-xl border border-slate-800 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2 sm:gap-2.5 min-w-0 pr-2">
            <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-violet-400 animate-ping shrink-0" />
            <div className="p-1 rounded-lg bg-slate-800 shrink-0">
              {getPresentationIcon(presentationType)}
            </div>
            <div className="min-w-0">
              <span className="text-[11px] sm:text-xs font-bold text-slate-100 flex items-center gap-1 sm:gap-1.5 truncate">
                ...
                {roomState?.editorName && (
                  <span className="text-amber-400 text-[10px] sm:text-xs font-normal hidden sm:inline">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            {isPresenter && (
              <button
                type="button"
                onClick={onStopPresenting}
                className="px-2.5 sm:px-3 py-1 rounded-xl font-bold text-[10px] sm:text-xs bg-rose-600/20 text-rose-300 hover:bg-rose-600/30 border border-rose-500/40 flex items-center gap-1.5 transition-colors"
              >
                <Square className="w-2.5 h-2.5 sm:w-3 sm:h-3 fill-current" />
